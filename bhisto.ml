...
let rec app_init z idx funh argh =
  if idx < 0 then
    bytes_map (add_char z) argh
  else if funh.[idx] = '\000' then
    app_init (succ_char z) (idx-1) funh argh
  else
    let len_newh = Bytes.length argh+idx+1 in
    let newh = Bytes.make len_newh z in
    app_main z idx (len_newh-1) newh funh argh

and app_main z fidx nidx funh argh newh =
  if fidx < 0 then begin
    for i = 0 to Bytes.length-1 do
      Bytes.set newh i (argh.[i] + z)
    done;
    newh
  end else
    let c = Bytes.get fidx funh in
    if c < z then begin
      Bytes.set newh (idx+1) 
